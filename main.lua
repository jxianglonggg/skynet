...
skynet.start(function()
    ...
    local function swap_c()
        for v in w:each "vector" do v.x, v.y = v.y, v.x end
    end

    local function swap_lua()
        for _, v in ipairs(t) do v.x, v.y = v.y, v.x end
    end
    ...
    w:new{vector = {x = 3, y = 4}, id = 100}

    table.insert(t, {x = 3, y = 4})

    w:new{vector = {x = 5, y = 6}, mark = true}
    ...
    local function csum() return test.sum(context) end
    print("csum = ", csum())

    local function luasum()
        local s = 0
        for v in w:each "vector" do s = s + v.x + v.y end
        return s
    end
    ...
    local function luanativesum()
        local s = 0
        for _, v in ipairs(t) do s = s + v.x + v.y end
        return s
    end
