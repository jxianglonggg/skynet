...
local function launch_slave(auth_handler,command_handler)
	local function auth(fd, addr)
		...
		local ok, server, uid, pass, id, sdk, osv, session_key =  pcall(auth_handler, token, addr)
		print("launch_slave", ok, server, uid, secret, pass, id,sdk,osv,session_key)
		return ok, server, uid, secret, pass, id, sdk, osv, session_key
	end
...
local function accept(conf, s, fd, addr)
	...
	print("s=", s)
	local ok, server, uid, secret, token, id,sdk, osv, session_key = skynet.call(s, "lua",  fd, addr)
	...
	print("accept3")
	local ok, err = pcall(conf.login_handler, server, uid, secret, addr, token, id,sdk,osv,session_key)
	-- unlock login
	user_login[uid] = nil
	if ok then
		print("accept4",err)
